...
namespace Tjl.DownloadHelper.Core
{
   public class MediaInfoFile
   {
      ...
      public MediaInfoFile(string mediaFileDirBase, string filePathTemplate, string mediaDirRelativeWithoutYoutubeId, Uri youTubeUri)
      {
         ...
         FilePathTargetFull = Path.Combine(mediaFileDirBase, DirTargetRelative, $"{DirTargetRelative}.txt");
      }
      ...
      public static string GetDirTargetRelative(string filenameWithoutPathAndPostfix, Uri youTubeUri)
      {
         ...
         return $"{filenameWithoutPathAndPostfix}_YouTube_{utId}";
      }
